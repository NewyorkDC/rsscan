...
app.get('/api/etfs', async (req, res) => {
  try { const data=await fetchAllETFs(); res.json({ok:true,data,updatedAt:new Date().toISOString(),count:data.length}); }
...
// 내 종목 API
app.use('/api/mystocks', myStocksRouter);
...
app.post('/api/alerts/test', async (req, res) => {
  ...
  if (!token||!chatId) return res.status(400).json({ok:false,error:'token/chatId 필수'});
  try {
    await makeBot(token).sendMessage(chatId,`🔔 <b>RSSCAN 테스트 알림</b>\n\n✅ 텔레그램 연동 성공!`,{parse_mode:'HTML'});
    res.json({ok:true});
...
async function sendAlerts(data) {
  for (const [chatId,cfg] of alertConfigs) {
    try {
      ...
      const msg=`📊 <b>RSSCAN 일일 알림</b>\n${new Date().toLocaleDateString('ko-KR')}\n\n${lines}\n\n<i>RS ≥ ${cfg.threshold}</i>`;
      await makeBot(cfg.token).sendMessage(chatId,msg,{parse_mode:'HTML'});
    } catch(err){}
  }
}

cron.schedule('5 16 * * 1-5', async()=>{ try{await sendAlerts(await fetchAllETFs());}catch(e){} },{timezone:'America/New_York'});
