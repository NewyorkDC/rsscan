...
const state = {
  ...
  customTickers: JSON.parse(localStorage.getItem('customTickers') || '[]'),
  sortKey: 'rsLineScore', sortAsc: false,
  stageFilter: 'all', phaseFilter: 'all',
  sectorFilter: 'all',
...
window.switchMode = function(mode) {
  state.mode = mode;
  $('tabETF').classList.toggle('active', mode==='etf');
  $('tabStock').classList.toggle('active', mode==='stock');
  $('etfFilterBar').style.display = mode==='etf'?'block':'none';
  $('stockFilterBar').style.display = mode==='stock'?'block':'none';
  $('thCat').textContent = mode==='etf'?'자산군':'섹터';

  if (mode==='stock' && !state.stockData.length) {
    loadStocks();
  } else {
    applyFilters();
  }
...
async function loadData() {
  ...
  state.loading = true;
  $('tableBody').innerHTML = '<tr><td colspan="15" class="loading-row"><div class="spinner"></div> 야후 파이낸스 데이터 수집 중... (30~60초)</td></tr>';
...
function applyFilters() {
  let d = state.mode==='etf' ? [...state.etfData] : [...state.stockData];
  ...
  state.filtered = d;
  renderTable();
  const cntEl = state.mode==='etf' ? $('resultCount') : $('stockResultCount');
  if (cntEl) cntEl.textContent = d.length;
}
...
function renderTable() {
  ...
  tbody.innerHTML = state.filtered.map(r => {
    ...
    const catLabel = state.mode==='stock' ? (r.sector||r.cat) : r.cat;
    return `<tr>
...
function saveAlertSettings() {
  ...
  $('alertBadge').style.display='flex'; $('alertBadge').textContent='✓';
  showToast('✓ 알림 설정 저장됨');
}
...
function renderSavedTickers() {
  const el=$('savedTickers');
  if (!state.customTickers.length) { el.innerHTML=''; return; }
  el.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px;">
    ${state.customTickers.map(t=>`
      <span style="background:var(--bg3);border:1px solid var(--border);padding:3px 10px;border-radius:5px;font-family:var(--mono);font-size:12px;display:flex;align-items:center;gap:6px;">
        ${t} <span onclick="removeTicker('${t}')" style="cursor:pointer;color:var(--red)">✕</span>
      </span>`).join('')}
  </div>`;
}

window.removeTicker = function(ticker) {
  state.customTickers = state.customTickers.filter(t=>t!==ticker);
  localStorage.setItem('customTickers', JSON.stringify(state.customTickers));
  renderSavedTickers();
};

function saveCustomTickers() {
  const input=$('customTickers').value.trim();
  if (!input) return;
  const newTickers = input.toUpperCase().split(',').map(t=>t.trim()).filter(Boolean);
  const merged = [...new Set([...state.customTickers, ...newTickers])];
  state.customTickers = merged;
  localStorage.setItem('customTickers', JSON.stringify(merged));
  $('customTickers').value='';
  renderSavedTickers();
  showToast(`✓ ${newTickers.length}개 종목 추가됨 (다음 새로고침 시 반영)`);
...
function bindEvents() {
  ...
  // 주식 섹터 필터
  document.querySelectorAll('[data-group="sector"]').forEach(btn => {
    btn.addEventListener('click', () => {
  ...
  $('stockSearchInput').addEventListener('input', e => { state.search=e.target.value.trim(); applyFilters(); });
  // 새로고침
  $('refreshBtn').addEventListener('click', () => {
    if (state.mode==='etf') loadData(); else loadStocks();
  });
  // 알림 모달
  ...
  // 내 종목 모달
  $('addStockBtn').addEventListener('click', () => { renderSavedTickers(); $('addStockModal').style.display='flex'; });
  $('addStockClose').addEventListener('click', () => $('addStockModal').style.display='none');
  $('saveTickersBtn').addEventListener('click', saveCustomTickers);
}
...
function init() {
  ...
  loadData();
  setInterval(() => { if(state.mode==='etf') loadData(false); }, 30*60*1000);
}
