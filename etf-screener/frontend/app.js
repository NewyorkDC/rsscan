...
window.switchMode = function(mode) {
  ...
  Object.keys(tabs).forEach(m => {
    const tab=$(tabs[m]), bar=$(bars[m]);
    if (tab) tab.classList.toggle('active', m===mode);
    if (bar) bar.style.display = m===mode ? 'block' : 'none';
  });
  const thCat=$('thCat'); if(thCat) thCat.textContent = mode==='etf'?'자산군':'섹터';
...
async function loadMyStocks() {
  ...
  if (state.loading) return;
  state.loading = true;
  $('tableBody').innerHTML = `<tr><td colspan="15" class="loading-row"><div class="spinner"></div> ${state.myTickers.join(', ')} 수집 중...</td></tr>`;
  try {
...
function renderTable() {
  ...
  if (!state.filtered.length) { tbody.innerHTML='<tr><td colspan="15" class="loading-row">검색 결과 없음</td></tr>'; return; }
...
function updateMarketPulse(data) {
  ...
  const above50 = data.filter(r=>r.stage==='S2'||r.stage==='S1→S2');
  const below50 = data.filter(r=>r.stage!=='S2'&&r.stage!=='S1→S2');
  const rs80    = data.filter(r=>r.rsLineScore>=80);
  const rs50    = data.filter(r=>r.rsLineScore<80);
  const pricePct = Math.round((above50.length/data.length)*100);
  const rsPct    = Math.round((rs80.length/data.length)*100);
  const health   = Math.round((pricePct*0.5)+(rsPct*0.5));

  pulseData = { data, pricePct, rsPct, health, above50, below50, rs80, rs50 };

  function gc(v) { return v>=70?'strong':v>=55?'good':v>=40?'neutral':v>=25?'weak':'danger'; }
  function gl(v) { return v>=70?'강한 강세':v>=55?'건강':v>=40?'중립':v>=25?'약세':'위험'; }
  function gs(v) { return v>=70?{icon:'🚀',text:'강세',color:'#22c55e'}:v>=55?{icon:'✅',text:'건강',color:'#84cc16'}:v>=40?{icon:'⚠️',text:'중립',color:'#f59e0b'}:v>=25?{icon:'📉',text:'약세',color:'#f97316'}:{icon:'🚨',text:'위험',color:'#ef4444'}; }

  const pc=gc(pricePct), rc=gc(rsPct), hc=gc(health), sig=gs(health);
  const el=id=>document.getElementById(id);
  if (!el('priceBreadth')) return;

  el('priceBreadth').textContent=pricePct+'%'; el('priceBreadth').className=`pulse-value health-${pc}`;
  el('priceBreadthSub').textContent=`${above50.length}/${data.length}개 · 50MA 상회`;
  el('priceBreadthBar').style.width=pricePct+'%'; el('priceBreadthBar').className=`pulse-bar-fill bar-${pc}`;

  el('rsBreadth').textContent=rsPct+'%'; el('rsBreadth').className=`pulse-value health-${rc}`;
  el('rsBreadthSub').textContent=`${rs80.length}/${data.length}개 · RS ≥ 80`;
  el('rsBreadthBar').style.width=rsPct+'%'; el('rsBreadthBar').className=`pulse-bar-fill bar-${rc}`;

  el('healthScore').textContent=health+'%'; el('healthScore').className=`pulse-value health-${hc}`;
  el('healthLabel').textContent=gl(health);
  el('healthBar').style.width=health+'%'; el('healthBar').className=`pulse-bar-fill bar-${hc}`;

  el('signalDot').textContent=sig.icon; el('signalDot').style.borderColor=sig.color;
  el('signalText').textContent=sig.text; el('signalText').className=`pulse-signal-text health-${hc}`;
}

// ── Pulse 상세 모달 ──
window.openPulseDetail = function(type) {
  const modal=$('pulseModal'), title=$('pulseModalTitle'), body=$('pulseModalBody');
  if (!modal) return;
  const { pricePct, rsPct, health, above50, below50, rs80, rs50, data } = pulseData;

  function hBar(v, color) {
    return `<div style="background:var(--bg3);border-radius:4px;height:8px;margin:6px 0 10px;">
      <div style="width:${v}%;height:100%;background:${color};border-radius:4px;transition:width 0.5s;"></div></div>`;
  }
  function tList(items, limit=12) {
    if (!items.length) return '<span style="color:var(--text3);font-size:12px">해당 없음</span>';
    return items.slice(0,limit).map(r=>
      `<span style="display:inline-flex;align-items:center;gap:4px;background:var(--bg3);border:1px solid var(--border);padding:3px 8px;border-radius:5px;margin:2px;font-family:var(--mono);font-size:12px;">
        <span style="color:var(--accent);font-weight:600">${r.ticker}</span>
        <span style="color:var(--text3)">${r.rsLineScore}</span>
      </span>`
    ).join('') + (items.length>limit?`<div style="color:var(--text3);font-size:11px;margin-top:6px">+${items.length-limit}개</div>`:'');
  }
  function color(v) { return v>=70?'#22c55e':v>=55?'#84cc16':v>=40?'#f59e0b':v>=25?'#f97316':'#ef4444'; }
  ...
  if (type==='price') {
    ...
    body.innerHTML = `
      <div style="font-size:13px;color:var(--text2);margin-bottom:12px">
        ...
        전체 종목 중 <b>50일 이동평균선(50MA) 위</b>에 있는 종목 비율로, 단기 추세가 살아있는 종목이 얼마나 되는지 보여줘요.
      </div>
      <div style="background:var(--bg3);border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
          <span style="font-family:var(--mono);font-size:24px;font-weight:700;color:${color(pricePct)}">${pricePct}%</span>
          <span style="font-size:12px;color:var(--text3)">${above50.length}/${data.length}개 상회</span>
        </div>
        ${hBar(pricePct, color(pricePct))}
      ...
      <div style="background:var(--bg3);border-radius:8px;padding:10px;margin-bottom:12px;font-size:12px;color:var(--text2);line-height:1.7">
        💡 <b>읽는 법</b><br>
      ...
      <div style="margin-bottom:12px">
        <div style="font-size:11px;color:var(--text3);font-family:var(--mono);text-transform:uppercase;margin-bottom:8px">✅ 50MA 상회 (${above50.length}개)</div>
        ${tList(above50.sort((a,b)=>b.rsLineScore-a.rsLineScore))}
      </div>
      <div>
        <div style="font-size:11px;color:var(--text3);font-family:var(--mono);text-transform:uppercase;margin-bottom:8px">❌ 50MA 하회 (${below50.length}개)</div>
        ${tList(below50.sort((a,b)=>b.rsLineScore-a.rsLineScore))}
      </div>`;
    ...
    body.innerHTML = `
      <div style="font-size:13px;color:var(--text2);margin-bottom:12px">
        ...
        전체 종목 중 <b>RS 점수 80 이상</b>인 종목 비율로, SPY 대비 강한 상대강도를 가진 종목이 얼마나 넓게 퍼져있는지 보여줘요.
      </div>
      <div style="background:var(--bg3);border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
          <span style="font-family:var(--mono);font-size:24px;font-weight:700;color:${color(rsPct)}">${rsPct}%</span>
          <span style="font-size:12px;color:var(--text3)">${rs80.length}/${data.length}개 RS 80+</span>
        </div>
        ${hBar(rsPct, color(rsPct))}
        <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text3);font-family:var(--mono)">
          <span>0% 위험</span><span>25%</span><span>40%</span><span>55%</span><span>70%+ 강세</span>
        </div>
      </div>
      ...
      <div style="margin-bottom:12px">
        <div style="font-size:11px;color:var(--text3);font-family:var(--mono);text-transform:uppercase;margin-bottom:8px">🔥 RS 80+ (${rs80.length}개)</div>
        ${tList(rs80.sort((a,b)=>b.rsLineScore-a.rsLineScore), 15)}
      </div>
      <div>
        <div style="font-size:11px;color:var(--text3);font-family:var(--mono);text-transform:uppercase;margin-bottom:8px">💤 RS 80 미만 (${rs50.length}개)</div>
        ${tList(rs50.sort((a,b)=>b.rsLineScore-a.rsLineScore))}
      </div>`;
    ...
    body.innerHTML = `
      <div style="text-align:center;padding:16px;margin-bottom:16px;background:var(--bg3);border-radius:10px">
        <div style="font-size:44px;margin-bottom:8px">${sig.icon}</div>
        <div style="font-size:30px;font-family:var(--mono);font-weight:700;color:${sig.color}">${health}%</div>
        <div style="font-size:15px;color:${sig.color};font-weight:600;margin:4px 0">${sig.text}</div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
        <div style="background:var(--bg3);border-radius:8px;padding:12px;text-align:center">
          ...
          <div style="font-size:22px;font-family:var(--mono);font-weight:600;color:${color(pricePct)}">${pricePct}%</div>
          <div style="font-size:11px;color:var(--text3)">50MA 상회</div>
        </div>
        <div style="background:var(--bg3);border-radius:8px;padding:12px;text-align:center">
          <div style="font-size:11px;color:var(--text3);margin-bottom:4px">🔺 RS 폭</div>
          <div style="font-size:22px;font-family:var(--mono);font-weight:600;color:${color(rsPct)}">${rsPct}%</div>
      ...
      <div style="background:var(--bg3);border-radius:8px;padding:12px;margin-bottom:12px;font-size:12px;color:var(--text2);line-height:1.8">
        ...
        <div><span style="color:#f59e0b">⚠️ 40~55%</span> 중립 — 방향성 확인 필요</div>
      ...
      <div style="background:var(--bg3);border-radius:8px;padding:12px;font-size:12px;color:var(--text2);line-height:1.6">
        ...
        ${pricePct>=70&&rsPct<40?'<br><br>⚠️ <b>주의:</b> 가격 폭은 높지만 RS 폭이 낮습니다. 소수 대형주가 지수를 끌어올리는 패턴일 수 있습니다.':
          pricePct>=55&&rsPct>=55?'<br><br>✅ 두 지표 모두 건강권 이상입니다. 건강한 상승장 신호입니다.':''}
      </div>`;
  }
...
function renderMyTickers() {
  ...
  el.innerHTML = state.myTickers.map(t=>
    `<span style="background:var(--amber-dim);border:1px solid var(--amber);color:var(--amber);padding:2px 10px;border-radius:5px;font-family:var(--mono);font-size:12px;display:inline-flex;align-items:center;gap:5px;">
...
function renderCurrentTickers() {
  ...
  el.innerHTML = state.myTickers.map(t=>
    `<span style="background:var(--bg3);border:1px solid var(--border);padding:3px 10px;border-radius:5px;font-family:var(--mono);font-size:12px;display:inline-flex;align-items:center;gap:6px;">
...
function bindEvents() {
  ...
  // 내 종목 모달
  const openBtn=$('openAddModal');   if(openBtn) openBtn.addEventListener('click',()=>{ renderCurrentTickers(); $('addTickerModal').style.display='flex'; });
  const closeBtn=$('addTickerClose'); if(closeBtn) closeBtn.addEventListener('click',()=>$('addTickerModal').style.display='none');
  const addBtn=$('addTickerBtn');    if(addBtn) addBtn.addEventListener('click',addMyTickers);
  const inp=$('newTickerInput');     if(inp) inp.addEventListener('keydown',e=>{ if(e.key==='Enter') addMyTickers(); });
  const loadBtn=$('loadMyBtn');      if(loadBtn) loadBtn.addEventListener('click',()=>{ $('addTickerModal').style.display='none'; loadMyStocks(); });
}
